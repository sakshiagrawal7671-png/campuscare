...
$where .= buildSearchClause('warden', $search, $params, [
    'u.name',
    'u.email',
    'u.phone',
    'h.hostel_name',
...
$countSql = 'SELECT COUNT(*)
             FROM users u
             LEFT JOIN hostel_wardens hw ON hw.warden_id = u.id
             LEFT JOIN hostels h ON h.id = hw.hostel_id' . $where;
...
$sql = 'SELECT u.id,
        ...
        FROM users u
        LEFT JOIN hostel_wardens hw ON hw.warden_id = u.id
        LEFT JOIN hostels h ON h.id = hw.hostel_id
        LEFT JOIN complaints c ON c.assigned_to = u.id'
        . $where .
        ' GROUP BY u.id, u.name, u.email, u.phone, u.status, h.id, h.hostel_name
